"""Utilities for saving generated occupied voxel centers as PLY point clouds."""

import os
import numpy as np
import torch


def coords_to_points(final_coords: torch.Tensor, depth: int):
    """Convert integer voxel coordinates [x, y, z, batch_id] to centers in [-1, 1]^3."""
    if final_coords is None or final_coords.numel() == 0:
        return np.zeros((0, 3), dtype=np.float32), np.zeros((0,), dtype=np.int64)

    coords = final_coords.detach().cpu().numpy()
    xyz = coords[:, :3].astype(np.float32)
    batch_ids = coords[:, 3].astype(np.int64)
    scale = float(2 ** depth)
    points = (xyz + 0.5) / scale
    points = points * 2.0 - 1.0
    return points, batch_ids


def save_pointcloud_ply(points: np.ndarray, save_path: str):
    """Save a point cloud as ASCII PLY. Each point is one occupied voxel center."""
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    with open(save_path, "w", encoding="utf-8") as f:
        f.write("ply\n")
        f.write("format ascii 1.0\n")
        f.write(f"element vertex {len(points)}\n")
        f.write("property float x\n")
        f.write("property float y\n")
        f.write("property float z\n")
        f.write("end_header\n")
        for p in points:
            f.write(f"{p[0]:.6f} {p[1]:.6f} {p[2]:.6f}\n")
